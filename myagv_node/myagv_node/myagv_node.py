...
class MyCar(object):
    ...
    def readSpeed(self):
        ...
        #print("{:.3f}".format(self.vx), "{:.3f}".format(self.vy), "{:.3f}".format(self.vtheta))
        ...
        #print("{:.3f}".format(self.ax), "{:.3f}".format(self.ay), "{:.3f}".format(self.az))
        ...
        #print("{:.3f}".format(self.wx), "{:.3f}".format(self.wy), "{:.3f}".format(self.wz))
    ...
    def write_speed(self, movex, movey, rot):
        if movex == 10 and movey == 10 and rot == 10:
            ...
            while True:
                ret = self.sp.readinto(buf_header)
                if ret != 1:
                    continue
                if buf_header[0] != self.header[0]:
                    continue
                ...
                if time.time() - start_time > 3:
                    self.get_logger().error("Get Voltage timeout")
                    break
            ...
            x_send = int(movex * 100) + 128
            y_send = int(movey * 100) + 128
            rot_send = int(rot * 100) + 128
...
class MinimalPublisher(Node):
    ...
    def execute(self):
        ...
        #quat_tf = tf_transformations.quaternion_from_euler(0, 0, self.theta)
        odom_quat = Quaternion()
        odom_quat.x = 0.0 #quat_tf[0]
        odom_quat.y = 0.0 # quat_tf[1]
        odom_quat.z = 0.0 #quat_tf[2]
        odom_quat.w = 0.0 #quat_tf[3]

        # Set translation and rotation in odom_trans
